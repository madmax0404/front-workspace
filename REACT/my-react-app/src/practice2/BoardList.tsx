...
function BoardList({ list, setList }: { list: typeof boardList, setList: React.Dispatch<React.SetStateAction<typeof boardList>> }) {
    ...
    const sortByDateAsc = () => {
        setList([...list].sort((a, b) => new Date(a.boardDate).getTime() - new Date(b.boardDate).getTime()));
    };
    const sortByDateDesc = () => {
        setList([...list].sort((a, b) => new Date(b.boardDate).getTime() - new Date(a.boardDate).getTime()));
    };
                ...
                <tbody>
                    {
                        ...
                        list.map((board) => (
                            <tr key={board.boardNo}>
                                ...
                                <td><button type="button" onClick={() => boardDelete(board.boardNo)}>삭제</button></td>
                            </tr>
                        ))
                    }
                </tbody>
