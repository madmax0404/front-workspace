...
function BoardRegister({ list, setList }: { list: typeof boardList, setList: React.Dispatch<React.SetStateAction<typeof boardList>> }) {
    ...
    const [board, setBoard] = useState({boardTitle:"", boardWriter:"", boardContent:""});
    ...
    const onRegisterHandler = () => {
        ...
        let newBoardDate = formatDate(new Date());

        let newList = [...list];
        newList.push({...board, boardNo:newBoardNo, boardDate:newBoardDate});

        setList(newList);
        setBoard({boardTitle: "", boardWriter: "", boardContent: ""});
    };
                ...
                <table className='enroll-table'>
                    <thead>
                        <tr>
                            <th>제목</th>
                            <td colSpan={3}>
                                <input type='text' name='boardTitle' onChange={onInputHandler} value={board.boardTitle}/>
                            </td>
                        </tr>
                        <tr>
                            <th>작성자</th>
                            <td colSpan={3}>
                                <input type='text' name='boardWriter' onChange={onInputHandler} value={board.boardWriter} />
                            </td>
                        </tr>
                        <tr>
                            <th>글내용</th>
                            <td colSpan={3} style={{ height: "200px" }}>
                                <textarea name='boardContent' onChange={onInputHandler} value={board.boardContent}></textarea>
                            </td>
                        </tr>
                        <tr>
